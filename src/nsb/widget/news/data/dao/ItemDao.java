...
import nsb.widget.news.utils.image.ImageUtils;
...
public class ItemDao extends AbstractDao<Item> {
	...
	public long insert(Item item) {
		try {
			...
			Long pubDate = item.getPubdate() != null ? item.getPubdate()
					.getTime() : null;

			if (pubDate != null)
				values.put("pub_date", pubDate); //$NON-NLS-1$
			if (item.getUpdateDate() != null)
				values.put("update_date", item.getUpdateDate().getTime()); //$NON-NLS-1$
			else if (pubDate != null)
				values.put("update_date", pubDate); //$NON-NLS-1$

			values.put(
					"image_url", item.getImage() != null ? item.getImage().toString() : null); //$NON-NLS-1$
			...
			long id = database.getWritableDatabase().insert(
					"item", null, values); //$NON-NLS-1$
	...
	public List<Item> getItems(int limit, boolean unreadOnly,
			boolean distribute, long... feedIds) {
		List<Item> items = new ArrayList<Item>();
		String uo = unreadOnly ? " (i.read is null or i.read=0) and " : ""; //$NON-NLS-1$ //$NON-NLS-2$

		String sql = "select " + //$NON-NLS-1$
				" i._id, i.feed_id, i.url, i.guid, i.title, i.author, i.pub_date, i.update_date, "
				+ //$NON-NLS-1$
				" i.description, i.image_url, i.original_source, i.original_author, i.read, i.favorite "
				+ //$NON-NLS-1$
		...
		if (distribute) {
			sql += " ( " + //$NON-NLS-1$
					"   select count(*) from item as i1 "
					+ //$NON-NLS-1$
					"   where i.feed_id = i1.feed_id and i1.pub_date > i.pub_date "
					+ //$NON-NLS-1$
					"   order by i1.update_date desc, i1.pub_date desc, i1.oid desc "
					+ //$NON-NLS-1$
	...
	public List<Item> getItemsById(long... itemIds) {
		List<Item> items = new ArrayList<Item>();
		String sql = "select " + //$NON-NLS-1$
				" _id, feed_id, url, guid, title, author, pub_date, update_date, "
				+ //$NON-NLS-1$
				" description, image_url, original_source, original_author, read, favorite "
				+ //$NON-NLS-1$
				" from item " + //$NON-NLS-1$
	...
	private Item getItem(Cursor cursor) {
		...
		try {
			if (!TextUtils.isEmpty(imageUrl))
				item.setImage(new URL(imageUrl));
		} catch (MalformedURLException e) {
			Log.e(Constants.PACKAGE,
					"Unable to parse back item image url: " + url, e); //$NON-NLS-1$
		}
	...
	public void markRead(Item item) {
		...
		database.executeUpdate(
				"update item set read=? where _id=?", new Object[] { b ? 1 : 0, item.getId() }); //$NON-NLS-1$
		item.setRead(b);
	}
	...
	public void markRead(long[] ids) {
		...
		database.executeUpdate(
				"update item set read=1 where _id in (" + placeHolders + ")", params); //$NON-NLS-1$ //$NON-NLS-2$
	}

	public void updateImage(Item item) {
		database.executeUpdate(
				"update item set image_url=? where _id=?", new Object[] { item.getImage().toString(), item.getId() }); //$NON-NLS-1$
	}
	...
	public void markFavorite(Item item) {
		...
		database.executeUpdate(
				"update item set favorite=? where _id=?", new Object[] { b ? 1 : 0, item.getId() }); //$NON-NLS-1$
		item.setFavorite(b);
	}
